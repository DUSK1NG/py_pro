...
启动 `app_styled.py` 后，页面分为“参数面板”和“结果工作区”两部分。左侧“工作模式”可以切换：

| 工作模式 | 主要用途 | 主要输入 |
| --- | --- | --- |
| 基础理论分析 | 快速计算标准简支梁 | 荷载类型、梁长、材料、截面和单位 |
| 教材题求解器 | 解答多荷载、多支座材料力学习题 | 支座表、集中力表、均布荷载表和梁参数 |

### 基础理论分析界面

左侧参数面板依次提供：

- **荷载类型**：跨中集中力、任意位置集中力、满跨均布荷载；
- **几何与材料**：梁长、长度单位、弹性模量、弹性模量单位和截面惯性矩；
- **截面属性**：矩形、圆形或自定义截面；
- **荷载输入**：荷载大小、单位和作用位置。

点击“开始计算”后，结果区显示最大剪力、最大弯矩、最大挠度和位置，并提供剪力图、弯矩图、挠度图和报告导出入口。

### 教材题求解器界面

选择“教材题求解器”后，页面按教材解题顺序展示：

1. **梁参数**：梁长 `L`、弹性模量 `E` 和截面惯性矩 `I`；
2. **支座表**：输入位置和 `pin`、`roller`、`fixed`、`free` 类型；
3. **集中力表**：输入每个集中力的位置和带符号大小；
4. **均布荷载表**：输入起点、终点和荷载强度；
5. 点击“求解教材题”，系统自动判断使用解析解或 FEM 数值解；
6. **结果区**：输入摘要、支座反力、平衡校核、剪力/弯矩/挠度图、分段 `V(x)`/`M(x)`、极值、受力简图数据和 FEM 节点结果；
7. **求解步骤与 warnings**：查看边界条件、符号约定、适用假设和需要复核的提示。

教材题输入使用内部单位 mm、N、MPa、mm⁴；向上荷载为正，向下荷载和向下挠度为负。标准左铰右滚简支梁和端部固定悬臂梁使用解析解，其他有效约束组合自动进入 FEM。

### 图像测量与数据分析

理论计算完成后，结果工作区还可以继续使用：

- **图像测量**：上传未加载图和加载图，进行标记点识别、像素标定和挠度曲线提取；
